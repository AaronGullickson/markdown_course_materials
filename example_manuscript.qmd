...
#citeproc: true # ensure that pandoc's citation system is used in all formats
bibliography: references.bib
csl: https://www.zotero.org/styles/apa-numeric-superscript-brackets
...
format: 
  html:
    ...
    theme: united              # bootswatch built-in theme 
    css: resources/css/my_styles.css    # css style
    # ensure fonts are installed on system by checking 
    # systemfonts::system_fonts()
    #mainfont: NewPeninimMT # font for main body
  ...
  pdf:
    ...
    # systemfonts::system_fonts()
    #mainfont: Papyrus # font for main body
    #sansfont: ComicSansMS       # font for headers
    #monofont: Monaco          # font for code
    #fontsize: 10pt            # font size (typically 10pt, 11pt, or 12pt)
    #linestretch: 2            # line spacing (1 for single, 2 for double)
    #geometry: margin=0.25in    # controls the margin size
    #papersize: a4         # controls the paper type
  #aog-article-pdf:
  #  fig-pos: "!t"
  #  keep-tex: false
  #  shorttitle: Example Document
...
#  ref-hyperlink: true    # (default is true)
  fig-labels: roman     # (default is arabic)
  tbl-labels: alpha A    # (default is arabic)
  eq-labels: alpha a      # (default is arabic)
...
#| results: asis
#| eval: false
cat("This is <b>html</b> code. <br>How does it look?")
```
...
[^2]: This is another footnote. This one has math.

    $$\sum (x_i-\bar{x})^2$$ Adipiscing habitasse integer sed fusce phasellus morbi libero mauris scelerisque fusce! Phasellus proin sem potenti torquent potenti ultrices orci bibendum morbi. Venenatis in vitae platea ligula orci dictumst pulvinar eleifend id ligula? Nisi vel mus tellus porttitor justo class cras orci est. Iaculis pretium tristique euismod facilisis vestibulum luctus lobortis quam tellus quis cum facilisis. Curabitur est parturient habitant proin elementum nascetur.
...
```{r}
#| label: tbl-penguins
#| tbl-cap: Some data on some penguins
#| tbl-pos: "p"

head(palmerpenguins::penguins) |>
  select(species, bill_length_mm, body_mass_g, sex) |>
  gt() |>
  sub_missing(missing_text = "-") |>
  fmt_number(bill_length_mm, decimals = 2, 
             pattern = "{x} mm") |>
  fmt_number(body_mass_g, decimals = 0, sep_mark = ",", use_seps = FALSE,
             pattern = "{x}g") |>
...
@Tbl-summary shows the descriptive statistics for my analytical dataset, separated by boys and girls.

```{r}
#| label: tbl-summary
#| tbl-cap: Summary statistics for Add Health data
#| tbl-pos: "p"
popularity |>
  tbl_summary(by=sex, 
              #type=list(nsports="continuous"),
              ...
              label=list(indegree ~ "Friend nominations",
                         ...
                         honorsociety ~ "Honor society member",
                         alcoholuse ~ "Alcohol use",
                         smoker ~ "Smoking behavior",
                         bandchoir ~ "Band/Choir member",
                         academicclub ~ "Academic club member",
                         sweight ~ "Sample weight",
                         parentinc ~ "Parent income ($1000s)",
                         nsports ~ "Number of sports played")) |>
  as_gt() |>
  tab_options(latex.use_longtable = FALSE) # set to true to disable floating
...
{{< video https://www.youtube.com/watch?v=-ccNkksrfls >}}

Diego Maradona's "Hand of God", World Cup 1986

:::

@fig-maradona shows Maradona was onside.
...
popularity |>
  ...
  tab_spanner("Variables", c(m_nom, p_drink, p_smoke)) |>
  ...
  tab_footnote(footnote = "A footnote in a body cell.",
               locations = cells_body(rows = 3:4, columns = m_nom),
               placement = "right") |>
  tab_source_note(md("*Source*: National Survey of Adolescent to Adult Health, Wave 1, 1997. Later waves are not available in this analysis. All results are unweighted.")) |>
  tab_source_note("Here is another note where you could say things.") |>
  tab_options(table.width = "100%", footnotes.marks = c("a","b","c","d"))
```
...
#| fig-cap: Two figures together on a separate float page
#| fig-height: 3.5
#| fig-pos: "p"
#| #layout: [[50, 50]]
#| layout-nrow: 2
#| fig-subcap: 
#|   - Distribution of number of friend nominations received in Add Health data.
#|   - Association between parental income and friend nominations by gender.
popularity |>
  ggplot(aes(x=indegree))+
  geom_histogram(binwidth = 1, center=0.5, 
                 color="grey30", fill="grey60")+
...
![You can have sloths, too.](resources/images/sloth1.png){#fig-sloth}
