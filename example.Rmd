...
```{r example-class, results="asis", eval=FALSE}
dt <- popularity %>%
  group_by(grade) %>%
...
dt %>%
  kbl(caption="Sample size and descriptive statistics, Add Health data, Wave 1",
      position="t", 
      escape=TRUE, #set this to false if using superscripts, but it will only knit to one output type
      align = c("l",rep("r", 4)),
...
See Table \@ref(tab:desc-stats-kable).

```{r desc-stats-kable, eval=TRUE, echo=TRUE}
popularity %>% 
  group_by(grade) %>%
  summarize(n=comma(n()), m_nom=round(mean(indegree), 2), 
         p_drink=percent(mean(alcoholuse=="Drinker"), accuracy=0.1),
...
See Figure \@ref(fig:dist-nominations).
...
knitreg(list(model1, model2, model3),
        ...
        custom.coef.names=c("Intercept","Smokes frequently","Drinks frequently",
                              "Parental income (\\$1000)", "Male",
                              "Black","Latino","API","Other","AIAN",
                              "GPA in four courses","Honor society member"),
        custom.model.names=c("(1)","(2)","(3)"),
        include.adjrs=FALSE,
        custom.header=list("Number of friend nominations"=1:3),
        groups=list("Gender (ref. female)"=5, "Race (ref. white)"=6:10),
        dcolumn=TRUE,
        single.row=TRUE,
        # The code below omits some coefficients and adds some custom
        # summary rows to explain what else is in the model. If you 
        # uncomment it, you will need to change custom.coef.names to just
        # the drinking and smoking labels
        # omit.coef="race|parentinc|sex|GPA|honor",
        # custom.gof.rows=list("Control for race?"=rep("Yes",3),
        #                      "Control for gender?"=rep("Yes",3),
        #                      "Control for parental income?"=rep("Yes",3),
        #                      "Control for GPA?"=rep("Yes",3),
        #                      "Control for honor society?"=rep("Yes",3)),
        custom.gof.names=c("R-squared","N"),
        # Three part table will allow for wrapped notes in the table
        # but you must start the custom note with "\\item" to work properly
        threeparttable=TRUE,
        custom.note="\\item %stars. Robust clustered standard errors in parenthesis. Data come from Add Health, Wave 1 respondents.")
```

See Table \@ref(tab:model-table).
...
knitreg(list(model1, model2, model3),
        digits=3,
        caption="(\\#tab:model-table)Linear models predicting the number of friend nominations recieved, Add Health data",
        caption.above=TRUE,
        booktabs=TRUE,
        float.pos = "t",
        dcolumn=TRUE,
